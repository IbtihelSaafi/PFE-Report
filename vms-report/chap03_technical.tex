...
\begin{itemize}
  ...
  \item \textbf{Werkzeug security helpers}: \texttt{generate\_password\_hash} and \texttt{check\_password\_hash}
...
CREATE TABLE findings (
    ...
    UNIQUE (asset_id, plugin_id, port, protocol) -- dedup key for re-imports
);
...
def import_tenable_rows(rows: list[dict], sla_days_map: dict):
    ...
    sla_days_map format: {"Critical": 7, "High": 15, ...}
    """
    now = datetime.utcnow()
...
def _compute_half_sla_timestamp(finding):
...
def send_half_sla_reminders(app):
    ...
    for finding in findings:
        half_point = _compute_half_sla_timestamp(finding)
        ...
        if finding.status not in {"NEW", "IN_PROGRESS", "ASSIGNED"}:
            continue
        # _send_half_sla_one is defined in app.py and logs Notification + EmailDelivery
...
\begin{itemize}
  ...
  \item \textbf{Session Security}: signed Flask session cookies with server-side role verification on protected routes
